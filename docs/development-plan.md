...
- [ ] 编写 install.sh 主脚本框架
  ...
  - [ ] 错误处理和回滚机制
  - [ ] 组件调用框架（先实现空函数占位）
    - [ ] install_prometheus（占位函数）
    - [ ] install_grafana（占位函数）
    - [ ] install_influxdb（占位函数）
    - [ ] install_node_exporter（占位函数）
    - [ ] install_jdk（占位函数）
    - [ ] install_jmeter（占位函数）
    - [ ] install_sysctl（占位函数）
    - [ ] config_ssh（占位函数）
